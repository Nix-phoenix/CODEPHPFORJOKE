...
$products = $conn->query("SELECT p_id, p_name, category, type, unit, price FROM Product ORDER BY p_name ASC")->fetch_all(MYSQLI_ASSOC);

// Fetch import list
$stmt = $conn->prepare("SELECT pod.pod_id AS id, DATE(po.date) AS date, po.po_id AS orderRef,
                               p.p_id, p.p_name AS product, p.category, p.type, p.unit,
                               pod.qty, pod.price AS unitPrice
                        FROM PurchaseOrderDetail pod
                        JOIN PurchaseOrder po ON po.po_id = pod.po_id
                        JOIN Product p ON p.p_id = pod.p_id
                        WHERE p.p_name LIKE ? OR po.po_id LIKE ?
...
<div id="importModal" class="modal">
    <div class="modal-content">
        ...
        <form id="importForm">
            ...
            <select id="orderRef" name="orderRef" class="gpg-input" required><?php foreach ($purchaseOrders as $po) {
                                                                                    echo "<option value='{$po['po_id']}'>{$po['po_id']}</option>";
                                                                                } ?></select>
            <label for="productName">ຊື່ສິນຄ້າ:</label>
            <select id="productId" name="productId" class="gpg-input" required onchange="fillProductDetails()"><?php foreach ($products as $p) {
                                                                                                                    echo "<option value='{$p['p_id']}' data-category='{$p['category']}' data-type='{$p['type']}' data-unit='{$p['unit']}' data-price='{$p['price']}'>{$p['p_name']}</option>";
                                                                                                                } ?></select>
            <label for="productCategory">ປະເພດ:</label>
            <input type="text" id="productCategory" name="productCategory" class="gpg-input" readonly>
            <label for="productType">ຊະນິດ:</label>
            <input type="text" id="productType" name="productType" class="gpg-input" readonly>
            <label for="productUnit">ຫນ່ວຍ:</label>
            <input type="text" id="productUnit" name="productUnit" class="gpg-input" readonly>
...
<script>
    ...
    function fillProductDetails() {
        ...
        document.getElementById('productType').value = opt.dataset.type || '';
        document.getElementById('productUnit').value = opt.dataset.unit || '';
    ...
    function editImport(id) {
        ...
        document.getElementById('productCategory').value = imp.category;
        document.getElementById('productType').value = imp.type;
        document.getElementById('productUnit').value = imp.unit;
    ...
    function renderTable() {
        ...
        imports.forEach((imp, i) => {
            tbody.insertRow().innerHTML = `<td>${i+1}</td><td>${new Date(imp.date).toLocaleDateString('en-GB')}</td><td>${imp.orderRef}</td><td>${imp.product}</td><td>${imp.category}</td><td>${imp.type}</td><td>${imp.unit}</td><td>${imp.qty}</td><td>${Number(imp.unitPrice).toLocaleString()} ກິບ</td><td>${total(imp).toLocaleString()} ກິບ</td><td><button class=\"dashboard-edit-btn\" onclick=\"editImport(${imp.id})\">ແກ້ໄຂ</button> <button class=\"dashboard-delete-btn\" onclick=\"deleteImport(${imp.id})\">ລົບ</button></td>`;
        });
    }
    document.getElementById('importForm').addEventListener('submit', function(e) {
        ...
        const data = {
            po_id: parseInt(this.orderRef.value),
    ...
    window.onclick = e => {
        if (e.target === document.getElementById('importModal')) closeModal();
    };
